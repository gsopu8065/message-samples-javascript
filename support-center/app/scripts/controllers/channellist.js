  ...
  .controller('ChannellistCtrl', function ($scope, authService, navService, channelService, $state, Alerts, notify) {
    ...
    var listener = new Max.EventListener('receivedMessageListener', function(mmxMessage) {
      ...
      if (mmxMessage.sender.userId != Max.getCurrentUser().userId
        ...
        && mmxMessage.channel.name != 'sapdemo_announcements'
        ...
        Max.Channel.getChannelSummary(supportedChannels, 10, 1).success(function(channelSummaries) {
          ...
          Max.User.getUsersByUserIds(ownerIds).success(function(owners) {
            ...
            for(var i = 0; i < channelSummaries.length; ++i) {
              ...
              if (channelSummaries[i].channelName == 'sapdemo_announcements') {
                channelSummaries[i].displayName = 'Broadcast Messages';
              } else if (isDefaultName(channelSummaries[i].channel.summary, channelSummaries[i].owner.userName)) {
    ...
    $scope.refreshForumList = function() {
      ...
      Max.Channel.findPublicChannels(null, 1000).success(function(channels) {
        for (var i=0;i<channels.length;++i) {
          if (channels[i].name == 'sapdemo_announcements') {
            channelService.broadcastChannel = channels[i];
          }
          if (channels[i].name == 'sapdemo_chat') {
            channelService.chatChannel = channels[i];
          }
        }
        if (channelService.broadcastChannel && channelService.chatChannel && (!channelService.broadcastChannel.isSubscribed || !channelService.chatChannel.isSubscribed)) {
          channelService.broadcastChannel.subscribe().success(function() {
            channelService.chatChannel.subscribe().success(function() {
              $scope.refreshChannelList();
            });
          });
        } else {
    ...
    function sortChannelSummary() {
      $scope.data.channelSummaries.sort(function(a, b) {
        return (a.lastPublishedTime > b.lastPublishedTime || a.channelName == 'sapdemo_announcements')
